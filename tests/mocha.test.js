...
describe("BP is created and Notification is recieved", () => {
  it("Should Create BP in mock application", (done) => {
    ...
    chai.request(config.mock.url).post(config.mock.api).set(header).send(payload)
      .end((err, res) => {
        try {     
...
function runNext() {
  ...
  describe("Handle Draft", () => {
    ...
    it("Payload for verification Status", (done) => {
      let headers = {
        "Content-Type": "application/json",
        "Authorization": "bearer " + xsuaa_access_token
      };
  ...
  describe("Confirm verification Status", () => {
  ...
  describe("BP is deleted", () => {
    it("Delete created BP from mock application", (done) => {
      chai.request(config.mock.url).delete(`${config.mock.api}('${payload.BusinessPartner}')`)
        .end((err, res) => {
          try {
    ...
    it("Delete created BP from validation app", (done) => {
      chai.request(config.service_domain).delete(`sales/Notifications(ID=${BPNotificationID},IsActiveEntity=true)`)
        .set('Authorization', 'bearer ' + xsuaa_access_token).end((err, response) => {
          try {
            expect(response.should.have.status(204))
            done();
          } catch (err) {
            console.error(err);
            done(err)
